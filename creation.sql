...
-- Drop existing cluster and index
DROP INDEX cluster_index;
DROP CLUSTER client_info_cluster;


-- Recreate the cluster with refined keys
CREATE CLUSTER client_info_cluster (
    username VARCHAR2(30),
    orderdate DATE
) SIZE 4096 TABLESPACE TAB_16K;


-- Recreate the index on the cluster
CREATE INDEX cluster_index ON CLUSTER client_info_cluster;
...
--DROP CLUSTER posts_cluster;
--DROP INDEX cluster2_index;
...
--CREATE INDEX cluster2_index ON CLUSTER posts_cluster;
...
--CLUSTER posts_cluster (username,postdate,barCode,product,score,title,text,likes,endorsed);
...
) CLUSTER client_info_cluster (username, orderdate);
    -- CLUSTER
--CLUSTER client_info_cluster (orderdate, username, town, country);
...
--CREATE INDEX usernameorders_idx
--ON Orders_Clients(username);




--CREATE INDEX usernamelines_idx
--ON Client_Lines(username);
